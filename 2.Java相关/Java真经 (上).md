...
- HashMap要变为线程安全，可以用Collections工具类：Map m = Collections.synchronizedMap(new HashMap()); 或者用ConcurrentHashMap。并发环境下，ConcurrentHashMap 效率较Collections.synchronizedMap()更高。
...
- HashTable是线程安全的，**它的大部分方法如put()、get()都加了synchronized锁，保证只有一个线程执行该方法**。当Hashtable的大小增加到一定的时候，性能会急剧下降，因为迭代时需要被锁定很长的时间，建议使用CcHashMap。
...
```
Arrays.asList不支持add/remove；另外注意Arrays.asList(new int[]{1,2,3})和asList(new Integer[]{1,2,3})的区别
```
