...
•  源码级掌握，扩容，红黑树，最小树化容量，hash冲突解决，有些面试官会提出发自灵魂的审问，比如为什么是红黑树，别的树不可以吗；为什么8的时候树化，4不可以吗，等等

#### 1.1.1 HashMap的底层

底层是：数组+链表+红黑树

- 容量*负载因子：默认16，0.75，扩容时容量是2n，最大容量2的31次方。
- 转树：桶中元素超过8个，且桶的容量超过64。树转链表则为6。 （效率：O（n）提高到O（logn））

- 允许使用 null键和 null值，null键是放在数组的第一位。
- 同步方式：Map m = Collections.synchronizedMap(new HashMap(...));

```json
为什么是红黑树而不是AVL树，因为红黑树的添加、删除和查找表现相对较好，而AVL树的查找更快，但增删慢。
```

#### 1.1.2 HashMap的put()方法 

- 对Key求Hash值，然后再计算下标。（return (key == null) ? 0 : (h = key.hashCode()) ^ (h >>> 16)）
- 如果数组为空，调用resize() 方法进行初始化数组。
- 如果没有碰橦，直接放入桶中
- 如果碰撞了，以链表的方式插入**链表尾部**。
- 如果链表长度超过阀值( TREEFY THRESHOLD==8)，就把链表转成红黑树。
- 如果节点已经存在就替换旧值。
- 如果新增操作，且桶满了(容量*加载因子)，就需要 resize()

```
* 目前对resize的了解：table==null时，调用resize初始化table。元素数量>=threshold时，调用resize扩容。
* HashMap生成的链表在jdk1.8之前是插入头部的，在jdk1.8中是插入尾部的。因为红黑树使用尾插法。
```

#### 1.1.3 HashMap线程非安全

* 多线程情况下，多线程同时添加元素的时候可能会丢失数据。因为对hashmap进行put()操作会引起扩容，各自生成新的数组并rehash后赋给该map底层的数组table，结果最终只有最后一个线程生成的新数组被赋给table变量，其他线程的均会丢失。而且当某些线程已经完成赋值而其他线程刚开始的时候，就会用已经被赋值的table作为原始数组，这样也会有问题。
* resize而引起死循环（JDK1.8已经不会出现该问题），这种情况发生在JDK1.7 中HashMap自动扩容时，当2个线程同时检测到元素个数超过允许的容量（数组大小 * 负载因子） 。此时2个线程会在put()方法中调用了resize()，两个线程同时修改一个链表结构会产生一个循环链表（JDK1.7中，会出现resize前后元素顺序倒置的情况）。接下来再想通过get()获取某一个元素，就会出现死循环。

```
1.7版本中，扩容是每个元素重新计算其哈希值以获取在新数组中索引下标，如果扩容后索引位置在新数组的索引位置与原数组中索引位置相同，则链表元素会发生倒置。
1.8版本中，新的位置为原位置或原位置+原数组长度。（如果是该位置只有一个元素，newTab[e.hash & (newCap - 1)] = e，如果是链表，则是根据公式(e.hash & oldCap)进行判断是否为0，原链表分为原位置链表+新位置链表）
```
...
#### 1.2.1 CcHashMap的基本概念

- ConcurrentHashMap键、值都**不允许为空**，为空则抛出NPE异常。
...
- HashTable是线程安全的，**它的大部分方法如put()、get()都加了synchronized锁，保证只有一个线程执行**。当Hashtable的大小增加到一定的时候，性能会急剧下降，因为迭代时需要被锁定很长的时间。
- HashTable初始容量为11, 0.75f，扩容时是2n+1，计算index的方法也不同。
- HashTable键、值都**不允许为空**，为空则抛出NPE异常。
- 1.8版本后，HashTable也没有引入红黑树。也就是说扩容是每个元素重新计算其哈希值以获取在新数组中索引下标。

```
HashTable取2n+1是因为hash函数使用的是标准的求模函数：(hash & 0x7FFFFFFF) % tab.length)，而HashMap是位运算。
```
...
CAS：全称Compare And Swap（比较与交换），解决多线程并行情况下使用锁造成性能损耗的一种机制。实现思想 CAS（V, A, B），V为内存地址、A为预期原值，B为新值。如果内存地址的值与预期原值相匹配，那么将该位置值更新为新值。否则，说明已经被其他线程更新，处理器不做任何操作；无论哪种情况，它都会在 CAS 指令之前返回该位置的值。而我们可以使用自旋锁，循环CAS，重新读取该变量再尝试再次修改该变量，也可以放弃操作。通常将 CAS 用于同步的方式是从地址 V 读取值 A，执行多步计算来获得新 值 B，然后使用 CAS 将 V 的值从 A 改为 B。如果 V 处的值尚未同时更改，则 CAS 操作成功。Java中的CAS操作都是通过sun包下Unsafe类实现，而Unsafe类中的方法都是native方法。
...
- 底层是一个数组，所以查询速度很快，默认容量是10，扩容时扩展为0.5倍
- 当数据满时，它会新建了一个数组；使用System.copy
- 非线程安全。用Collectionutils.synchronized(list)可以转为线程安全类。
- 遍历删除时，用迭代器删除，可以倒序删除。


```
LinkedList的底层是一个链表，所以增删的速度很快。（增删只改引用，不用移动）
```
...
String是final修饰的不可变类，底层是一个final的char数组，不可变。它是线程安全的。

StringBuilder是可变字符串类，是线程非安全的。
...
使用@interface可以生成自己的注解。注解一般是跟切面来配合使用。
